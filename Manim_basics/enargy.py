...
class HandDrawnWaveScene(Scene):
    def construct(self):
        # Use frame to animate background color over time
        ...
        # Jitter via offset tracker (no .animate on background_color)
        offset_tracker = ValueTracker(0.0)

        def dynamic_wave_updater(mob):
            # small horizontal jitter based on tracker
            dx = 0.2 * math.sin(2 * PI * offset_tracker.get_value())
            # use original y; only shift in x
        ...
        # Animate frame color instead of background_color.animate
        self.play(
            *rgb_animations,
            frame.animate.set_fill(color="#06061A", opacity=1.0),
            run_time=7.0,
            rate_func=smooth,
        )
